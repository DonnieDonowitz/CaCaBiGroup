...
ScreenRecord::ScreenRecord() : m_fps(30), m_vIndex(-1), m_aIndex(-1), m_vFmtCtx(nullptr), m_aFmtCtx(nullptr), m_oFmtCtx(nullptr), m_vDecodeCtx(nullptr), m_aDecodeCtx(nullptr), m_vEncodeCtx(nullptr), m_aEncodeCtx(nullptr), m_vFifoBuf(nullptr), m_aFifoBuf(nullptr), m_swsCtx(nullptr), m_swrCtx(nullptr), m_state(RecordState::NotStarted), m_vCurPts(0), m_aCurPts(0)
{
...
void ScreenRecord::Init(std::string path, int width, int height, std::string video, std::string audio)
{
    ...
    m_videoDevice = video;
    m_audioDevice = audio;
    std::cout << "Init completata" << std::endl;
}
...
{
    std::cout << "Sono dentro OpenVideo" << std::endl;
    int ret = -1;
#ifdef _WIN32
...
#else
    if (avformat_open_input(&m_vFmtCtx, "1:none", ifmt, &options) != 0)
    {
        return -1;
    }
#endif
...
{
    std::cout << "Sono dentro OpenAudio" << std::endl;
    int ret = -1;
    AVCodec *decoder = nullptr;
#ifdef _WIN32
...
#endif
    std::string test(":1");
    if (avformat_open_input(&m_aFmtCtx, test.c_str(), ifmt, nullptr) < 0)
    {
        return -1;
    }
...
std::string ScreenRecord::GetSpeakerDeviceName()
{
    ...
    while (hr = pEm->Next(1, &pM, &cFetched), hr == S_OK)
    {
        ...
        if (SUCCEEDED(hr))
        {
            VARIANT var;
            var.vt = VT_BSTR;
            hr = pBag->Read(L"FriendlyName", &var, NULL) if (hr == NOERROR)
            {
...
void ScreenRecord::FlushVideoDecoder()
{
    ...
    while (ret >= 0)
    {
...

        {
            ...
            m_cvVBufNotFull.wait(lk, [this]
                                 { return av_fifo_space(m_vFifoBuf) >= m_vOutFrameSize; });
        }
...
void ScreenRecord::FlushAudioDecoder()
{
    ...
    while (ret >= 0)
    {
...

        {
            ...
            m_cvABufNotFull.wait(lk, [newFrame, this]
...
void ScreenRecord::MuxThreadProc()
{
    ...
    while (1)
    {
                          ...
                          m_aCurPts, m_oFmtCtx->streams[m_aOutIndex]->time_base) <= 0)
        {
            ...
            {
                std::unique_lock<std::mutex> lk(m_mtxVBuf);
                m_cvVBufNotEmpty.wait(lk, [this]
                                      { return av_fifo_size(m_vFifoBuf) >= m_vOutFrameSize; });
            }
